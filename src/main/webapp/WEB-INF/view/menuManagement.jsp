	...
	<header>
		<div class="header-left">
			<a href="${pageContext.request.contextPath}/AdminMenuServlet"
				class="btn-header-back" title="管理メニューへ戻る">←</a>
			<h1 style="margin: 0; font-size: 22px;">メニュー管理（期間限定・追加・削除）</h1>
		</div>
	</header>
			...
			<form action="MenuManagementServlet" method="post"
				...
				<div class="input-group">
					<label>商品名</label> <input type="text" name="productName"
						placeholder="例: 冬の特選うどん">
				</div>
				<div class="input-group">
					<label>価格</label> <input type="text" name="productPrice"
						placeholder="例: 900">
				</div>
				<div class="input-group">
				...
				<div class="input-group">
					<label>画像</label> <input type="file" name="image" accept="image/*"
						required>
				</div>
				<div class="input-group">
					<label>商品説明</label>
				...
				<table>
					...
					<tbody>
						<c:forEach var="list" items="${mvm.products}">
							<tr>
								<td class="img-cell"><img
									src='assets/img/${list.productImage}' alt="画像"></td>
								<td><strong>${list.productName}</strong><br> <small
									style="color: #666;">ID: ${list.productId}</small></td>
								<td>${list.productPrice}円</td>
								<td>
									<div style="display: flex; gap: 8px;">
										<form action="EditProductServlet" method="get">
											<button type="submit" name="id" value="${list.productId}"
												class="btn-edit">編集</button>
										</form>
										<form action="MenuManagementServlet" method="post">
											<input type="hidden" name="action" value="delete"> 
											<input type="hidden" name="category" value="${currentCategory}">
											<button type="submit" name="id" value="${list.productId}"
												class="btn-del">削除</button>
										</form>	
									</div>
								</td>
							</tr>
